...
/// this position in at least two well-defined ways:
///
...
///   line layout needs.
///
/// * **LB19a under-suppression of opening quotes.** Unicode 15.1
///   split the old `× QU` rule into context-dependent LB19a/LB19b
///   variants, where `× QU_PI` now requires a specific East-Asian
///   left context that `AL` does not satisfy. Strictly applying the
///   table would allow line breaks immediately before opening
///   smart-quotes (`“`, `‘`) and guillemets (`«`) — typographically
///   undesirable. We therefore explicitly include `QU_PI` and
///   `QU_PF` so that opening punctuation continues to attach to
///   what follows at the SA seam, mirroring the pre-15.1 behavior.
///
...
/// hold here (LB7/LB9/LB11/LB12a/LB13/LB21/LB22), or — for
/// `QU_PI`/`QU_PF` — to a typographic intent the spec's
/// context-dependent rules no longer encode directly:
///
...
/// * **`LB12a`**:`× GL` — non-breaking glue (the SP/BA/HY exception
///   does not apply when the previous class is SA).
/// * **`LB13`**: `× CL`, `× CP`, `× EX`, `× IS`, `× SY`.
/// * **`LB19/LB19a`**: `× QU`, `× QU_PI`, `× QU_PF` — opening and
///   closing quotation marks attach to adjacent text; the post-15.1
///   East-Asian context restrictions are intentionally not applied
///   here (see above).
...
mod tests {
    ...
    /// these positions; the `dict_boundary_before_iteration_mark_override`
