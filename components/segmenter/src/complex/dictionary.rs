...
impl<R: RuleBreakType> Iterator for DictionaryBreakIterator<'_, '_, R> {
    type Item = usize;
        ...
        while let Some(next) = self.iter.next() {
            match trie_iter.next32(next.1.into()) {
                TrieResult::FinalValue(_) => {
                    return Some(next.0 + R::char_len(next.1));
                }
                TrieResult::Intermediate(_) => {
                    // Dictionary has to match with grapheme cluster segment.
                    // If not, we ignore it.
                    while last_grapheme_offset < next.0 + R::char_len(next.1) {
                    ...
                    if last_grapheme_offset != next.0 + R::char_len(next.1) {
                        continue;
                    }

                    intermediate_length = next.0 + R::char_len(next.1);
...
#[derive(Copy, Clone)]
pub(super) struct DictionarySegmenterBorrowed<'data> {
    dict: &'data UCharDictionaryBreakData<'data>,
    grapheme: GraphemeClusterSegmenterBorrowed<'data>,
}

impl<'data> DictionarySegmenterBorrowed<'data> {
    pub(super) fn new(
        dict: &'data UCharDictionaryBreakData<'data>,
        grapheme: GraphemeClusterSegmenterBorrowed<'data>,
    ) -> Self {
...
mod tests {
    ...
    fn cj_dictionary_test() {
        ...
        let dict_segmenter = DictionarySegmenterBorrowed::new(
