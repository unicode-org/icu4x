...
impl<'l, 's, Y: RuleBreakType<'l, 's> + ?Sized> Iterator for RuleBreakIterator<'l, 's, Y> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            ...
            println!("left={:02X} right={:02X} state={:02X}", left_prop, right_prop, break_state);
            println!("left={} right={}", STATE_NAMES[left_prop as usize], STATE_NAMES[right_prop as usize]);
