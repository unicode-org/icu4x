...
impl ConverterFactory {
    ...
    fn compute_offset(
        ...
        let output_conversion_factor_recip = output_conversion_info.factor_as_ratio().recip();
        Some((input_offset - output_offset) * output_conversion_factor_recip)
    }
    ...
    pub fn converter<T: Convertible>(
        ...
        let mut conversion_factor = IcuRatio::one();
        for &input_single_unit in input_unit.single_units.as_slice() {
            conversion_factor *= Self::compute_conversion_term(self, input_single_unit, 1)?;
        }

        if input_unit.constant_denominator() != 0 {
            conversion_factor /= IcuRatio::from_integer(input_unit.constant_denominator());
        }

        for &output_single_unit in output_unit.single_units.as_slice() {
            conversion_factor *= Self::compute_conversion_term(
                self,
                output_single_unit,
        ...
        if output_unit.constant_denominator() != 0 {
            if is_reciprocal {
                conversion_factor /= IcuRatio::from_integer(output_unit.constant_denominator());
            } else {
                conversion_factor *= IcuRatio::from_integer(output_unit.constant_denominator());
            }
        }
        ...
        let conversion_factor = T::ratio_from_ratio_bigint(conversion_factor.get_ratio())?;
        let proportional = ProportionalConverter { conversion_factor };
