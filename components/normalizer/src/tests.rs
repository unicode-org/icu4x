...
fn test_uts46_basic() {
    ...
    let normalizer: ComposingNormalizer =
        ComposingNormalizer::try_new_uts46_without_ignored_and_disallowed_unstable(&data_provider).unwrap();
    assert_eq!(normalizer.normalize("a\u{0308}"), "ä");
    assert_eq!(normalizer.normalize("A\u{0308}"), "ä");
    assert_eq!(normalizer.normalize("e\u{0323}\u{0302}"), "ệ");
    assert_eq!(normalizer.normalize("E\u{0323}\u{0302}"), "ệ");
...
fn test_conformance() {
    ...
    let nfd: DecomposingNormalizer = DecomposingNormalizer::try_new_nfd_unstable(&data_provider).unwrap();
    let nfkd: DecomposingNormalizer = DecomposingNormalizer::try_new_nfkd_unstable(&data_provider).unwrap();
    let nfc: ComposingNormalizer = ComposingNormalizer::try_new_nfc_unstable(&data_provider).unwrap();
    let nfkc: ComposingNormalizer = ComposingNormalizer::try_new_nfkc_unstable(&data_provider).unwrap();
