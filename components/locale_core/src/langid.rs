...
impl LanguageIdentifier {
    ...
    pub fn canonicalize_utf8(input: &[u8]) -> Result<Cow<str>, ParseError> {
        ...
        if lang_id.writeable_cmp_bytes(input) == Ordering::Equal {
            ...
            Ok(lang_id.write_to_string().into_owned().into())
        }
    }
    ...
    pub fn canonicalize(input: &str) -> Result<Cow<str>, ParseError> {
        let lang_id = Self::try_from_utf8(input.as_bytes())?;
        if lang_id.writeable_cmp_bytes(input.as_bytes()) == Ordering::Equal {
            Ok(input.into())
        } else {
            Ok(lang_id.write_to_string().into_owned().into())
        }
