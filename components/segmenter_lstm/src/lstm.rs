...
use ndarray::{Array1, Array2, ArrayBase, ViewRepr, Dim};
...
impl Lstm {
    ...
    fn compute_hc(&self, x_t:ArrayBase<ViewRepr<&f32>, Dim<[usize; 1]>>, h_tm1:&Array1::<f32>, c_tm1:&Array1::<f32>, 
        warr:ArrayBase<ViewRepr<&f32>, Dim<[usize; 2]>>, uarr:ArrayBase<ViewRepr<&f32>, Dim<[usize; 2]>>, 
        barr:ArrayBase<ViewRepr<&f32>, Dim<[usize; 1]>>) -> (Array1::<f32>, Array1::<f32>){    
    ...
    pub fn word_segmenter(&self, input:&str) -> String {
        ...
        for i in 0..graph_clust.len() {
            let g = graph_clust[i];
            let g_id:i16 = if self.data.dic.contains_key(g) {self.data.dic[g]} else {graph_clust_len};
            let x_t = self.data.mat1.slice(ndarray::s![g_id as isize, ..]);
            let (new_h, new_c) = self.compute_hc(x_t, &h_fw, &c_fw, self.data.mat2.view(), 
            self.data.mat3.view(), self.data.mat4.view());
            h_fw = new_h;
            c_fw = new_c;
        ...
        for i in (0..graph_clust.len()).rev() {
            ...
            let x_t = self.data.mat1.slice(ndarray::s![g_id as isize, ..]);
            let (new_h, new_c) = self.compute_hc(x_t, &h_bw, &c_bw, self.data.mat5.view(), 
            self.data.mat6.view(), self.data.mat7.view());
            h_bw = new_h;
            c_bw = new_c;
