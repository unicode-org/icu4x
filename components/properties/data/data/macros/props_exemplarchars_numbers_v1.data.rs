    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_properties::provider::ExemplarCharactersNumbersV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_properties::provider::ExemplarCharactersNumbersV1Marker>, icu_provider::DataError> {
                ...
                static VALUES: [&<icu_properties::provider::ExemplarCharactersNumbersV1Marker as icu_provider::DataMarker>::Yokeable; 80usize] = [&AF, &AR, &AR_DZ, &AR_DZ, &AR_DZ, &AR_DZ, &AR_DZ, &AR_SA, &AR_SA, &AR_DZ, &AS, &AF, &AF, &BGC, &BGC, &AS, &AF, &AF, &AF, &AF, &DE_CH, &DE_CH, &AF, &AF, &AF, &AF, &ET, &EU, &FA, &FF_ADLM, &ET, &EU, &FR, &GU, &AR_DZ, &BGC, &AF, &AF, &DE_CH, &AF, &AF, &AF, &KN, &BGC, &KS, &AF, &ET, &AF, &ML, &AS, &BGC, &MY, &BGC, &ET, &OR, &PA, &AF, &PS, &AF, &BGC, &RM, &AF, &BGC, &SAT, &AF, &EU, &AF, &ET, &TA, &TE, &AF, &AF, &UND, &KS, &AF, &AF, &YUE, &YUE, &YUE, &YUE];
