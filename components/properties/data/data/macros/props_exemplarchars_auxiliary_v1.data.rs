    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_properties::provider::ExemplarCharactersAuxiliaryV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_properties::provider::ExemplarCharactersAuxiliaryV1Marker>, icu_provider::DataError> {
                ...
                static VALUES: [&<icu_properties::provider::ExemplarCharactersAuxiliaryV1Marker as icu_provider::DataMarker>::Yokeable; 132usize] = [&AF, &AR, &AR_MA, &AS, &AST, &AZ, &BE, &BG, &BGC, &BHO, &BN, &BR, &BRX, &BS, &CA, &CEB, &CS, &CV, &CY, &DA, &DE, &DOI, &DSB, &EL, &EN, &EN_ZA, &ES, &ET, &EU, &FA, &FA_AF, &FF_ADLM, &FI, &FIL, &FO, &FR, &FR_CA, &GA, &GD, &GL, &GU, &HA, &HA_NE, &HE, &HI, &HI_LATN, &BS, &HSB, &HU, &HY, &EN, &EN, &IG, &IS, &IT, &JA, &JV, &KA, &KEA, &KGP, &KM, &KN, &KO, &HI, &KS, &BRX, &KY, &LO, &LT, &LV, &MAI, &MI, &MK, &MN, &BRX, &HI, &MY, &HI, &NL, &NN, &NO, &BRX, &PA, &PCM, &PL, &PS, &PS_PK, &PT, &QU, &RAJ, &RM, &RO, &CV, &SA, &SC, &SD, &BRX, &SI, &SK, &SL, &SO, &AZ, &SR, &SR_LATN, &SU, &SV, &SW, &SW_CD, &BRX, &TE, &TG, &TH, &TI, &TK, &TO, &TR, &TT, &UK, &UND, &UR, &UZ, &TG, &VI, &WO, &EN, &YO, &YRL, &YUE, &YUE_HANS, &ZH, &ZH_HANT, &EN];
