    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_properties::provider::ExemplarCharactersPunctuationV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_properties::provider::ExemplarCharactersPunctuationV1Marker>, icu_provider::DataError> {
                ...
                static VALUES: [&<icu_properties::provider::ExemplarCharactersPunctuationV1Marker as icu_provider::DataMarker>::Yokeable; 93usize] = [&AF, &AM, &AR, &AS, &AF, &BE, &BG, &AF, &BS, &CA, &CS, &AF, &DA, &DE, &EL, &AF, &CA, &ET, &AF, &FA, &FI, &FIL, &FR, &AF, &GD, &CA, &AF, &HA, &HE, &HI, &AF, &HR, &HU, &HY, &AF, &IG, &IS, &IT, &JA, &JV, &KA, &KK, &KM, &KN, &KO, &AF, &DE, &AF, &LT, &LV, &MK, &AF, &AF, &KN, &AF, &MY, &NE, &AF, &NN, &NN, &AF, &PA, &AF, &PL, &PS, &AF, &RO, &DE, &SD, &AF, &CS, &SL, &AF, &SQ, &SR, &SR, &AF, &SW, &AF, &TE, &TH, &TK, &AF, &UK, &UR, &AF, &AF, &YO, &YUE, &YUE_HANS, &YUE_HANS, &YUE, &JV];
                ...
                match KEYS.binary_search_by(|k| locale.strict_cmp(k.as_bytes()).reverse()) {
                    ...
                    Err(_) => Err(icu_provider::DataErrorKind::MissingLocale.with_req(<icu_properties::provider::ExemplarCharactersPunctuationV1Marker as icu_provider::KeyedDataMarker>::KEY, req)),
                }
            }
        }
    };
