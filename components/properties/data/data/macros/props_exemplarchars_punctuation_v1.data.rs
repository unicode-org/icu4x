            ...
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_properties::provider::ExemplarCharactersPunctuationV1Marker>, icu_provider::DataError> {
                ...
                match {
                    ...
                    match ["af", "am", "ar", "as", "az", "be", "bg", "bn", "bs", "ca", "cs", "cy", "da", "de", "el", "en", "es", "et", "eu", "fa", "fi", "fil", "fr", "ga", "gd", "gl", "gu", "ha", "he", "hi", "hi-Latn", "hr", "hu", "hy", "id", "ig", "is", "it", "ja", "jv", "ka", "kk", "km", "kn", "ko", "kok", "ky", "lo", "lt", "lv", "mk", "ml", "mn", "mr", "ms", "my", "ne", "nl", "nn", "no", "or", "pa", "pcm", "pl", "ps", "pt", "ro", "ru", "sd", "si", "sk", "sl", "so", "sq", "sr", "sr-Latn", "sv", "sw", "ta", "te", "th", "tk", "tr", "uk", "ur", "uz", "vi", "yo", "yue", "yue-Hans", "zh", "zh-Hant", "zu"].binary_search_by(|k| locale.strict_cmp(k.as_bytes()).reverse()) {
                        Ok(i) => Ok(*unsafe { [&AF, &AM, &AR, &AS, &AF, &BE, &BG, &AF, &BS, &CA, &CS, &AF, &DA, &DE, &EL, &AF, &CA, &ET, &AF, &FA, &FI, &FIL, &FR, &AF, &GD, &CA, &AF, &HA, &HE, &HI, &AF, &HR, &HU, &HY, &AF, &IG, &IS, &IT, &JA, &JV, &KA, &KK, &KM, &KN, &KO, &AF, &DE, &AF, &LT, &LV, &MK, &AF, &AF, &KN, &AF, &MY, &NE, &AF, &NN, &NN, &AF, &PA, &AF, &PL, &PS, &AF, &RO, &DE, &SD, &AF, &CS, &SL, &AF, &SQ, &SR, &SR, &AF, &SW, &AF, &TE, &TH, &TK, &AF, &UK, &UR, &AF, &AF, &YO, &YUE, &YUE_HANS, &YUE_HANS, &YUE, &JV].get_unchecked(i) }),
                        Err(_) => Err(icu_provider::DataErrorKind::MissingLocale),
                    }
                } {
                    Ok(payload) => Ok(icu_provider::DataResponse { metadata: Default::default(), payload: Some(icu_provider::DataPayload::from_static_ref(payload)) }),
                    Err(e) => Err(e.with_req(<icu_properties::provider::ExemplarCharactersPunctuationV1Marker as icu_provider::KeyedDataMarker>::KEY, req)),
                }
            }
        }
    };
