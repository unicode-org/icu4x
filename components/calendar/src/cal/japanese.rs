...
///
/// The calendar scheme using months and days identical to the Gregorian calendar was adopted starting with January 1st, 6 Meiji (1873-01-01), so years before 6 Meiji are proleptic. This implementation uses the Meiji era proleptically back to and including 1868-10-23 and uses the Gregorian eras for 1868-10-22 and earlier dates. For a variant that uses approximations of historical Japanese eras proleptically, check out [`JapaneseExtended`].
///
...
/// differently using the Japanese era system.
///
/// The calendar scheme using months and days identical to the Gregorian calendar was adopted starting with January 1st, 6 Meiji (1873-01-01), so years before 6 Meiji are proleptic. This implementation uses approximations of earlier Japanese eras proleptically and uses the Gregorian eras for even earlier dates that don't have an approximate Japanese era. For a variant whose Japanese eras start with Meiji, check out [`Japanese`].
///
/// This type can be used with [`Date`] to represent dates in this calendar.
///
/// [Japanese calendar]: https://en.wikipedia.org/wiki/Japanese_calendar
