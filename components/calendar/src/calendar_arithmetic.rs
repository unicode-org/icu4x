...
mod tests {
    ...
    fn test_validity_ranges() {
        ...
        #[rustfmt::skip]
        let lowest_rds = [
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::JANUARY.code(), 1, Buddhist).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::new(1).code(), 1, ChineseTraditional::new()).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::THOUT.code(), 1, Coptic).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::MESKEREM.code(), 1, Ethiopian::new_with_era_style(EthiopianEraStyle::AmeteAlem)).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::MESKEREM.code(), 1, Ethiopian::new_with_era_style(EthiopianEraStyle::AmeteMihret)).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::JANUARY.code(), 1, Gregorian).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::TISHRI.code(), 1, Hebrew).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::MUHARRAM.code(), 1, Hijri::new_umm_al_qura()).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::MUHARRAM.code(), 1, Hijri::new_tabular(HijriTabularLeapYears::TypeII, HijriTabularEpoch::Thursday)).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::CHAITRA.code(), 1, Indian).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::JANUARY.code(), 1, Iso).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::JANUARY.code(), 1, Japanese::new()).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::JANUARY.code(), 1, Julian).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::new(1).code(), 1, KoreanTraditional::new()).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::FARVARDIN.code(), 1, Persian).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.start(), Month::JANUARY.code(), 1, Roc).unwrap().to_rata_die(),
        ];

        #[rustfmt::skip]
        let highest_rds = [
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::DECEMBER.code(), 31, Buddhist).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::new(12).code(), 30, ChineseTraditional::new()).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::NASIE.code(), 5, Coptic).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::PAGUMEN.code(), 5, Ethiopian::new_with_era_style(EthiopianEraStyle::AmeteAlem)).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::PAGUMEN.code(), 5, Ethiopian::new_with_era_style(EthiopianEraStyle::AmeteMihret)).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::DECEMBER.code(), 31, Gregorian).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::ELUL.code(), 29, Hebrew).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::DHUL_HIJJAH.code(), 30, Hijri::new_umm_al_qura()).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::DHUL_HIJJAH.code(), 30, Hijri::new_tabular(HijriTabularLeapYears::TypeII, HijriTabularEpoch::Thursday)).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::PHALGUNA.code(), 30, Indian).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::DECEMBER.code(), 31, Iso).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::DECEMBER.code(), 31, Japanese::new()).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::DECEMBER.code(), 31, Julian).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::new(12).code(), 30, KoreanTraditional::new()).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::ESFAND.code(), 30, Persian).unwrap().to_rata_die(),
            Date::try_new_from_codes(None, *VALID_YEAR_RANGE.end(), Month::DECEMBER.code(), 31, Roc).unwrap().to_rata_die(),
        ];
