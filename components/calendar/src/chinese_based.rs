...
pub(crate) trait ChineseBased<C: ChineseBased<C> + CalendarArithmetic> {
    ...
    fn major_solar_term_from_fixed(date: RataDie) -> i32 {
        let moment: Moment = date.as_moment();
        let location = Self::location(date);
    ...
    /// Based on functions from _Calendrical Calculations_ by Reingold & Dershowitz.
    /// Lisp reference code: https://github.com/EdReingold/calendar-code2/blob/main/calendar.l#L5273-L5281
    fn minor_solar_term_from_fixed(date: RataDie) -> i32 {
        let moment: Moment = date.as_moment();
        let location = Self::location(date);
        let universal: Moment = Location::universal_from_standard(moment, location);
        let solar_longitude = i64_to_i32(Astronomical::solar_longitude(universal) as i64);
    ...
    fn new_moon_on_or_after(moment: Moment) -> RataDie {
        let new_moon_moment = Astronomical::new_moon_at_or_after(Self::midnight(moment));
        let location = Self::location(new_moon_moment.as_rata_die());
    ...
    fn new_moon_before(moment: Moment) -> RataDie {
    ...
    fn midnight(date: Moment) -> Moment {
        Location::universal_from_standard(date, Self::location(date.as_rata_die()))
    }
    ...
    fn new_year_in_sui(date: RataDie) -> RataDie {
    ...
    fn winter_solstice_on_or_before(date: RataDie) -> RataDie {
    ...
    fn new_year_on_or_before_fixed_date(date: RataDie) -> RataDie {
    ...
    fn chinese_based_date_from_fixed(date: RataDie) -> Date<C> {
        ...
        Self::new_chinese_based_date(year, month, day)
    }
    ...
    fn fixed_mid_year_from_year(elapsed_years: i32) -> RataDie {
        ...
        Self::EPOCH + ((((cycle - 1) * 60 + year - 1) as f64 + 0.5) * MEAN_TROPICAL_YEAR) as i64
    }
