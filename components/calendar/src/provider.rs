...
impl JapaneseEras {
    ...
    pub(crate) fn last_after_reiwa(self) -> Option<(EraStartDate, TinyAsciiStr<8>, u8)> {
        self.0.map(PackedEra::unpack)
    }
}
...
impl databake::Bake for JapaneseEras {
    fn bake(&self, ctx: &databake::CrateEnv) -> databake::TokenStream {
        ...
        if let Some((era_start_date, code, idx)) = self.last_after_reiwa() {
            let era_start_date = era_start_date.bake(ctx);
            let code = code.bake(ctx);
            let idx = idx.bake(ctx);
            databake::quote! {
                icu_calendar::provider::JapaneseEras::with_post_reiwa_era(#era_start_date, #code, #idx).unwrap()
            }
        } else {
            databake::quote! {
...
impl serde::Serialize for JapaneseEras {
        ...
        S: serde::Serializer,
    {
        ...
        Raw {
            dates_to_eras: if let Some((era_start_date, code, idx)) = self.last_after_reiwa() {
                core::iter::repeat_n((era_start_date, code.resize()), idx as usize - 1).collect()
            } else {
                ZeroVec::new()
            },
        }
...
fn japanese_eras_size() {
    ...
    assert_eq!(size_of::<JapaneseEras>(), 7);
...
fn japanese_eras_packing() {
    ...
    assert_eq!(JapaneseEras::up_to_reiwa().last_after_reiwa(), None);
}
...
#[test]
fn japanese_eras_legacy_deserialize() {
    let legacy_json = r#"{
        "dates_to_eras": [
        [
            {
    ...
    assert_eq!(
        serde_json::from_str::<JapaneseEras>(legacy_json).unwrap(),
        JapaneseEras::up_to_reiwa()
    );
