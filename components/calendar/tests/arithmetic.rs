...
#[test]
fn test_hebrew() {
    let m06z_20 = Date::try_new_from_codes(None, 5783, Month::ADAR.code(), 20, Hebrew).unwrap();
    let m05l_15 = Date::try_new_from_codes(None, 5784, Month::ADAR_I.code(), 15, Hebrew).unwrap();
    let m05l_30 = Date::try_new_from_codes(None, 5784, Month::ADAR_I.code(), 30, Hebrew).unwrap();
    let m06a_29 = Date::try_new_from_codes(None, 5784, Month::ADAR.code(), 29, Hebrew).unwrap();
    let m07a_10 = Date::try_new_from_codes(None, 5784, Month::NISAN.code(), 10, Hebrew).unwrap();
    let m06b_15 = Date::try_new_from_codes(None, 5785, Month::ADAR.code(), 15, Hebrew).unwrap();
    let m07b_20 = Date::try_new_from_codes(None, 5785, Month::NISAN.code(), 20, Hebrew).unwrap();
...
fn test_tricky_leap_months() {
    ...
    let date0 = hebrew_date(5783, Month::ADAR, 20);
    let duration0 = DateDuration::for_years(1);
    let date1 = date0
        .try_added_with_options(duration0, add_options)
        .unwrap();
    assert_eq!(date1, hebrew_date(5784, Month::ADAR, 20));
    ...
    let date2 = date1
        ...
        .unwrap();
    assert_eq!(date2, hebrew_date(5784, Month::ADAR_I, 20));
    ...
    let date3 = date2
        ...
        .unwrap();
    assert_eq!(date3, hebrew_date(5785, Month::NISAN, 20));
    ...
    let date4 = date1
        .try_added_with_options(duration2, add_options)
        .unwrap();
    assert_eq!(date4, hebrew_date(5785, Month::NISAN, 20));
