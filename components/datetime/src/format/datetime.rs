...
use crate::time_zone::{
    Bcp47IdFormat, GenericLocationFormat, GenericNonLocationFormat, Iso8601Format, IsoFormat,
    LocalizedOffsetFormat, Minutes, ResolvedNeoTimeZoneSkeleton, Seconds,
    SpecificNonLocationFormat,
};
    ...
    ZS: ZoneSymbols<'data>,
{
    ...
    Ok(match (field.symbol, field.length) {
        ...
        (FieldSymbol::TimeZone(symbol), length) => {
            let payloads = zone_symbols.map(|zs| zs.get_payloads()).unwrap_or_default();
