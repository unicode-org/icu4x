...
///
/// let mut dtz = TimeZoneCalculator::new().resolve(ZonedDateTime::try_from_str("2023-11-20T11:35:03+00:00[Europe/London]")
///     .unwrap()
...
/// let resolved = TimeZoneCalculator::new().resolve_at(zone, &DateTime::try_new_iso_datetime(2025, 1, 2, 0, 0, 0).unwrap());
///
/// // The pattern string contains lots of symbols including "E", "MMM", and "a",
...
/// // Missing data is filled in on a best-effort basis, and an error is signaled.
/// assert_try_writeable_parts_eq!(
///     names.with_pattern(&pattern).format(&resolved),
...
impl<C: CldrCalendar, R: DateTimeNamesMarker> TypedDateTimeNames<C, R> {
    ...
    /// use writeable::assert_try_writeable_eq;
    ///
    /// let mut zone_london_winter = TimeZoneCalculator::new().resolve(ZonedDateTime::try_from_str("2024-01-01T00:00:00+00:00[Europe/London]").unwrap()).zone();
    /// let mut zone_london_summer = TimeZoneCalculator::new().resolve(ZonedDateTime::try_from_str("2024-07-01T00:00:00+01:00[Europe/London]").unwrap()).zone();
    ///
    /// let mut names =
    ...
    R::Z: ZoneMarkers,
{
    ...
    /// use writeable::assert_try_writeable_eq;
    ///
    /// let mut london_winter = TimeZoneCalculator::new().resolve(ZonedDateTime::try_from_str("2024-01-01T00:00:00+00:00[Europe/London]").unwrap().to_calendar(Gregorian));
    /// let mut london_summer = TimeZoneCalculator::new().resolve(ZonedDateTime::try_from_str("2024-07-01T00:00:00+01:00[Europe/London]").unwrap().to_calendar(Gregorian));
    ///
    /// let mut names =
    ///     TypedDateTimeNames::<Gregorian, NeoTimeZoneSkeleton>::try_new(&locale!("en-GB").into())
    ///         .unwrap();
