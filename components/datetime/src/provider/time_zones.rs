...
pub struct MetazonePeriodV1<'data>(
    ...
    ///
    /// The second level key is a wall-clock time represented as
    /// the number of minutes since the local [`EPOCH`](icu_timezone::provider::EPOCH).
    /// It represents when the metazone started to be used.
    ///
    /// The value is a metazone ID, which is uppercase if the timezone used DST during
    /// the time it was in the metazone. Time zones that didn't use DST use the
    /// standard display name even in generic format.
    #[cfg_attr(feature = "serde", serde(borrow))]
