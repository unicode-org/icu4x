...
pub struct LocationsV1<'data> {
    /// The display location for each zone
    #[cfg_attr(feature = "serde", serde(borrow))]
    pub zones: ZeroMap<'data, TimeZoneBcp47Id, str>,
    /// The display name for an unknown time zone. This is not combined with the region pattern.
    #[cfg_attr(feature = "serde", serde(borrow))]
