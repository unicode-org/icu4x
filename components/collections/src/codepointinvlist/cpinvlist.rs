...
const BMP_INV_LIST_VEC: ZeroVec<PotentialCodePoint> = zerovec!(PotentialCodePoint; PotentialCodePoint::const_to_unaligned; [PotentialCodePoint::from_u24(0x0), PotentialCodePoint::from_u24(BMP_MAX + 1)]);

/// Represents the inversion list for all of the code points in the Unicode range.
const ALL_VEC: ZeroVec<PotentialCodePoint> = zerovec!(PotentialCodePoint; PotentialCodePoint::const_to_unaligned; [PotentialCodePoint::from_u24(0x0), PotentialCodePoint::from_u24((char::MAX as u32) + 1)]);
...
impl<'data> CodePointInversionList<'data> {
    ...
    /// use icu::collections::codepointinvlist::InvalidSetError;
    /// use potential_utf::PotentialCodePoint;
    /// let valid = [PotentialCodePoint::from_u24(0x0), PotentialCodePoint::from_u24(0x10000)];
    /// let result = CodePointInversionList::try_from_inversion_list_slice(&valid);
    /// assert!(matches!(result, CodePointInversionList));
    ///
    /// let invalid: Vec<PotentialCodePoint> = vec![PotentialCodePoint::from_u24(0x0), PotentialCodePoint::from_u24(0x80), PotentialCodePoint::from_u24(0x3)];
    /// let result =
    ...
    pub fn try_from_inversion_list_slice(
        inv_list: &'data [PotentialCodePoint],
    ) -> Result<Self, InvalidSetError> {
        let inv_list_zv: ZeroVec<PotentialCodePoint> = ZeroVec::from_slice_or_alloc(inv_list);
    ...
    /// let example =
    ///     CodePointInversionList::try_clone_from_inversion_list_slice(&example_list)
    ///         .unwrap();
    ...
    ///         .unwrap();
    /// let a_to_d =
    ///     CodePointInversionList::try_clone_from_inversion_list_slice(&[0x41, 0x45])
    ///         .unwrap();
    /// let f_to_t =
    ///     CodePointInversionList::try_clone_from_inversion_list_slice(&[0x46, 0x55])
    ///         .unwrap();
    /// let r_to_x =
    ///     CodePointInversionList::try_clone_from_inversion_list_slice(&[0x52, 0x58])
    ///         .unwrap();
...
mod tests {
    ...
    fn test_codepointinversionlist_try_from_vec_error() {
        ...
        let set = CodePointInversionList::try_clone_from_inversion_list_slice(&check);
    ...
    fn test_codepointinversionlist_contains_set_u() {
        ...
        let u = CodePointInversionList::try_clone_from_inversion_list_slice(&ex).unwrap();
        let inside = vec![0xF, 0x14, 0x2C, 0x31, 0x46, 0x50, 0x64, 0x6D];
        let s = CodePointInversionList::try_clone_from_inversion_list_slice(&inside).unwrap();
    ...
    fn test_codepointinversionlist_contains_set_u_false() {
        ...
        let s = CodePointInversionList::try_clone_from_inversion_list_slice(&outside).unwrap();
        assert!(!u.contains_set(&s));
    }
    ...
    fn test_codepointinversionlist_iter_ranges() {
        let ex = vec![0x41, 0x44, 0x45, 0x46, 0xD800, 0xD801];
        let set = CodePointInversionList::try_clone_from_inversion_list_slice(&ex).unwrap();
    ...
    fn test_serde_deserialize() {
        ...
        let exp_uniset =
            CodePointInversionList::try_clone_from_inversion_list_slice(&exp_inv_list).unwrap();
