...
use icu::plurals::{PluralCategory, PluralOperands, PluralRuleType, PluralRulesWithRanges};

#[test]
fn test_plural_ranges_raw() {
    assert_eq!(
        PluralRulesWithRanges::try_new_cardinal(&langid!("he").into())
            .unwrap()
...
fn test_plural_ranges_optimized_data() {
    assert_eq!(
        PluralRulesWithRanges::try_new_ordinal(&langid!("en").into())
            .unwrap()
...
fn test_plural_ranges_missing_data_fallback() {
    assert_eq!(
        PluralRulesWithRanges::try_new_cardinal(&langid!("nl").into())
            .unwrap()
...
fn test_plural_ranges_full() {
    let ranges =
        PluralRulesWithRanges::try_new(&langid!("sl").into(), PluralRuleType::Cardinal).unwrap();
