...
int main() {
    ...
    diplomat_result_box_ICU4XLocale_ICU4XLocaleParseError locale_result = ICU4XLocale_create_from_string("ar", 2);
    if (!locale_result.is_ok) {
        return 1;
    }
    ICU4XLocale* locale = locale_result.ok;
    ICU4XDataProvider* provider = ICU4XDataProvider_create_compiled();
    diplomat_result_box_ICU4XPluralRules_ICU4XDataError plural_result = ICU4XPluralRules_create_cardinal(provider, locale);
