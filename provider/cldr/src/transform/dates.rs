...
impl<'d, 's> DataProvider<'d, 's, gregory::DatesV1Marker> for DatesProvider<'d> {
    fn load_payload(
        &self,
        req: &DataRequest,
    ) -> Result<DataResponse<'d, 's, gregory::DatesV1Marker>, DataError> {
...
fn test_basic() {
    ...
    let cs_dates: DataPayload<gregory::DatesV1Marker> = provider
