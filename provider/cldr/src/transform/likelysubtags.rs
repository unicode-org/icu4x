...
impl<'d, 's> DataProvider<'d, 's, LikelySubtagsV1_M> for LikelySubtagsProvider<'d> {
    fn load_payload(
        &self,
        req: &DataRequest,
    ) -> Result<DataResponse<'d, 's, LikelySubtagsV1_M>, DataError> {
...
fn test_basic() {
    ...
    let result: DataPayload<LikelySubtagsV1_M> = provider
