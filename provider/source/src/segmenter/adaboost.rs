...
use icu::segmenter::provider::adaboost::{
    AdaboostData, SegmenterChineseAutoV1, SegmenterThaiAutoV1, ThaiAdaboostData,
};
...
impl RawThaiAdaboostData {
    fn try_convert(&self) -> Result<ThaiAdaboostData<'static>, DataError> {
        let weight_sum = sum_weights([
...
impl DataProvider<SegmenterThaiAutoV1> for SourceDataProvider {
    fn load(&self, req: DataRequest) -> Result<DataResponse<SegmenterThaiAutoV1>, DataError> {
        ...
        let raw = serde_json::from_str::<RawThaiAdaboostData>(THAI_MODEL_JSON).map_err(|e| {
...
mod tests {
    ...
    fn converts_built_in_thai_model() {
        let raw = serde_json::from_str::<RawThaiAdaboostData>(THAI_MODEL_JSON)
