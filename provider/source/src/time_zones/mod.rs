...
pub(crate) struct Caches {
    ...
    reverse_metazones: Cache<BTreeMap<(MetazoneId, MzVariant), Vec<TimeZoneBcp47Id>>>,
}
...
impl From<ZoneVariant> for MzVariant {
    fn from(value: ZoneVariant) -> Self {
        match value {
...
impl SourceDataProvider {
    fn reverse_metazones(
        &self,
    ) -> Result<&BTreeMap<(MetazoneId, MzVariant), Vec<TimeZoneBcp47Id>>, DataError> {
        self.cldr()?
            .tz_caches
            .reverse_metazones
            .get_or_init(|| {
                ...
                let mut reverse_metazones =
                    BTreeMap::<(MetazoneId, MzVariant), Vec<TimeZoneBcp47Id>>::new();
                ...
                for c in mz_period.list.iter0() {
                    ...
                    let mut mzs = c
                        .into_iter1_copied()
                        .map(|(k, v)| (i32::from_unaligned(*k), v))
                        .peekable();
                    ...
                    let horizon =
                        to_iso_minutes_since_epoch((self.timezone_horizon, Time::midnight()));
