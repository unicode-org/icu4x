...
use crate::marker::DynDataMarker;
...
///
/// The type of the data stored in [`DataPayload`] is determined by the [`DynDataMarker`] type parameter.
///
/// ## Accessing the data
///
...
/// ```
pub struct DataPayload<M: DynDataMarker>(pub(crate) DataPayloadInner<M>);
...
pub struct DataPayloadOr<M: DynDataMarker, O>(pub(crate) DataPayloadOrInner<M, O>);

pub(crate) enum DataPayloadInner<M: DynDataMarker> {
    Yoke(Yoke<M::Yokeable, CartableOptionPointer<CartInner>>),
    StaticRef(&'static M::Yokeable),
}

pub(crate) enum DataPayloadOrInner<M: DynDataMarker, O> {
    Yoke(Yoke<M::Yokeable, CartableOptionPointer<CartInner>>),
    Inner(DataPayloadOrInnerInner<M, O>),
}

pub(crate) enum DataPayloadOrInnerInner<M: DynDataMarker, O> {
    StaticRef(&'static M::Yokeable),
    Other(O),
}
    ...
    M: DynDataMarker,
{
    ...
    /// impl DynDataMarker for HelloWorldV1MessageMarker {
    ...
    /// Convert between two [`DynDataMarker`] types that are compatible with each other
    ...
    /// impl DynDataMarker for CustomHelloWorldV1Marker {
    ...
    where
        M2: DynDataMarker<Yokeable = M::Yokeable>,
    {
