...
macro_rules! impl_dyn_provider {
    ...
    ($provider:ty, { $($pat:pat $(if $guard:expr)? => $struct_m:ty),+, }, SERDE_SE, impl ConvertData) => {
        // If this fails to compile, enable the "serialize" feature on this crate.
        $crate::impl_dyn_provider!(
            $provider,
            { $($pat $(if $guard)? => $struct_m),+, },
    ...
    ($provider:ty, [ $($struct_m:ty),+, ], SERDE_SE, impl ConvertData) => {
        $crate::impl_dyn_provider!(
            $provider,
            [ $($struct_m),+, ],
            SERDE_SE
    ...
    ($provider:ty, { $($pat:pat $(if $guard:expr)? => $struct_m:ty),+, }, impl ConvertData) => {
        // If this fails to compile, enable the "datagen" feature on this crate.
        impl $crate::datagen::ConvertData<$crate::any::AnyMarker, $crate::serde::SerializeMarker> for $provider {
        ...
        impl $crate::datagen::ConvertData<$crate::serde::SerializeMarker, $crate::datagen::HeapStatsMarker> for $provider {
            fn convert(&self, key: $crate::ResourceKey, from: DataPayload<$crate::serde::SerializeMarker>) -> Result<$crate::DataPayload<$crate::datagen::HeapStatsMarker>, $crate::datagen::ReturnedPayloadError<$crate::serde::SerializeMarker>> {
                use $crate::datagen::ReturnedPayloadError;
                match key {
                    $(
                        $pat $(if $guard)? => {
                            let heap_stats = from.attempt_zero_copy_heap_size::<$struct_m>().map_err(|e| $crate::datagen::ReturnedPayloadError(from, e))?;
