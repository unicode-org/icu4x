...
#[doc(hidden)]
#[macro_export]
macro_rules! __singleton_propnames_from_sc_v1 {
    () => {
...
#[doc(hidden)]
#[macro_export]
macro_rules! __lookup_propnames_from_sc_v1 {
    ($ locale : expr) => {{
        static SINGLETON: <icu_properties::provider::ScriptNameToValueV1Marker as icu_provider::DataMarker>::Yokeable = singleton_propnames_from_sc_v1!();
        if $locale.is_empty() {
            Ok(&SINGLETON)
        } else {
...
/// Implement [`DataProvider<ScriptNameToValueV1Marker>`](icu_provider::DataProvider) on the given struct using the data
...
macro_rules! __impl_propnames_from_sc_v1 {
    ($ provider : path) => {
        #[clippy::msrv = "1.61"]
        impl icu_provider::DataProvider<icu_properties::provider::ScriptNameToValueV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_properties::provider::ScriptNameToValueV1Marker>, icu_provider::DataError> {
                match lookup_propnames_from_sc_v1!(req.locale) {
                    ...
                    Err(e) => Err(e.with_req(<icu_properties::provider::ScriptNameToValueV1Marker as icu_provider::KeyedDataMarker>::KEY, req)),
                }
            }
        }
    };
