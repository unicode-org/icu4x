...
#[doc(hidden)]
#[macro_export]
macro_rules! __lookup_props_exemplarchars_punctuation_v1 {
    ($ req : expr) => {
        ["ar", "ar-EG", "bn", "ccp", "en", "en-001", "en-ZA", "es", "es-AR", "fil", "fr", "ja", "ru", "sr", "sr-Cyrl", "sr-Latn", "th", "tr", "und"].binary_search_by(|k| $req.locale.strict_cmp(k.as_bytes()).reverse()).ok().map(|i| unsafe {
            ...
            *[&AR, &AR, &BN, &CCP, &BN, &BN, &BN, &ES, &ES, &FIL, &FR, &JA, &RU, &SR, &SR, &SR, &TH, &BN, &UND].get_unchecked(i)
        })
    };
}
    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_properties::provider::ExemplarCharactersPunctuationV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_properties::provider::ExemplarCharactersPunctuationV1Marker>, icu_provider::DataError> {
                lookup_props_exemplarchars_punctuation_v1!(req).map(icu_provider::prelude::zerofrom::ZeroFrom::zero_from).map(icu_provider::DataPayload::from_owned).map(|payload| icu_provider::DataResponse { metadata: Default::default(), payload: Some(payload) }).ok_or_else(|| icu_provider::DataErrorKind::MissingLocale.with_req(<icu_properties::provider::ExemplarCharactersPunctuationV1Marker as icu_provider::KeyedDataMarker>::KEY, req))
            }
        }
    };
}
