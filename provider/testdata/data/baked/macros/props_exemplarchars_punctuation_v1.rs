...
macro_rules! __impl_props_exemplarchars_punctuation_v1 {
    ($ provider : path) => {
        ...
        impl $provider {
            #[doc(hidden)]
            pub fn lookup_props_exemplarchars_punctuation_v1(locale: &icu_provider::DataLocale) -> Result<&'static <icu_properties::provider::ExemplarCharactersPunctuationV1Marker as icu_provider::DataMarker>::Yokeable, icu_provider::DataErrorKind> {
                ...
                match ["ar", "ar-EG", "bn", "ccp", "en", "en-001", "en-ZA", "es", "es-AR", "fil", "fr", "ja", "ru", "sr", "sr-Cyrl", "sr-Latn", "th", "tr", "und"].binary_search_by(|k| locale.strict_cmp(k.as_bytes()).reverse()) {
                    Ok(i) => Ok(*unsafe { [&AR, &AR, &BN, &CCP, &BN, &BN, &BN, &ES, &ES, &FIL, &FR, &JA, &RU, &SR, &SR, &SR, &TH, &BN, &UND].get_unchecked(i) }),
        ...
        impl icu_provider::DataProvider<icu_properties::provider::ExemplarCharactersPunctuationV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_properties::provider::ExemplarCharactersPunctuationV1Marker>, icu_provider::DataError> {
                match Self::lookup_props_exemplarchars_punctuation_v1(&req.locale) {
