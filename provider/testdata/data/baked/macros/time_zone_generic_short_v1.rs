...
macro_rules! __impl_time_zone_generic_short_v1 {
    ($ provider : path) => {
        ...
        impl $provider {
            #[doc(hidden)]
            pub fn lookup_time_zone_generic_short_v1(locale: &icu_provider::DataLocale) -> Result<&'static <icu_datetime::provider::time_zones::MetazoneGenericNamesShortV1Marker as icu_provider::DataMarker>::Yokeable, icu_provider::DataErrorKind> {
                ...
                match ["ar", "ar-EG", "bn", "ccp", "en", "en-001", "en-ZA", "es", "es-AR", "fil", "fr", "ja", "ru", "sr", "sr-Cyrl", "sr-Latn", "th", "tr", "und"].binary_search_by(|k| locale.strict_cmp(k.as_bytes()).reverse()) {
                    Ok(i) => Ok(*unsafe { [&AR, &AR, &BN, &BN, &EN, &EN_001, &EN_ZA, &ES, &ES_AR, &FIL, &FIL, &JA, &FIL, &ES, &ES, &ES, &FIL, &FIL, &FIL].get_unchecked(i) }),
        ...
        impl icu_provider::DataProvider<icu_datetime::provider::time_zones::MetazoneGenericNamesShortV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_datetime::provider::time_zones::MetazoneGenericNamesShortV1Marker>, icu_provider::DataError> {
                match Self::lookup_time_zone_generic_short_v1(&req.locale) {
                    ...
                    Err(e) => Err(e.with_req(<icu_datetime::provider::time_zones::MetazoneGenericNamesShortV1Marker as icu_provider::KeyedDataMarker>::KEY, req)),
                }
            }
        }
    };
