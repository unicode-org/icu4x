...
macro_rules! __impl_plurals_cardinal_v1 {
    ($ provider : path) => {
        ...
        impl $provider {
            #[doc(hidden)]
            pub fn lookup_plurals_cardinal_v1(locale: &icu_provider::DataLocale) -> Result<&'static <icu_plurals::provider::CardinalV1Marker as icu_provider::DataMarker>::Yokeable, icu_provider::DataErrorKind> {
                ...
                match ["ar", "bn", "en", "es", "fil", "fr", "ja", "ru", "sr", "th", "tr", "und"].binary_search_by(|k| locale.strict_cmp(k.as_bytes()).reverse()) {
                    Ok(i) => Ok(*unsafe { [&AR, &BN, &EN, &ES, &FIL, &FR, &JA, &RU, &SR, &JA, &TR, &JA].get_unchecked(i) }),
        ...
        impl icu_provider::DataProvider<icu_plurals::provider::CardinalV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_plurals::provider::CardinalV1Marker>, icu_provider::DataError> {
                match Self::lookup_plurals_cardinal_v1(&req.locale) {
                    ...
                    Err(e) => Err(e.with_req(<icu_plurals::provider::CardinalV1Marker as icu_provider::KeyedDataMarker>::KEY, req)),
                }
            }
        }
    };
