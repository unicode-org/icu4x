...
#[doc(hidden)]
#[macro_export]
macro_rules! __lookup_compactdecimal_long_v1 {
    ($ req : expr) => {
        ["ar", "ar-EG", "ar-EG-u-nu-latn", "ar-u-nu-latn", "bn", "bn-u-nu-latn", "ccp", "ccp-u-nu-latn", "en", "en-001", "en-ZA", "es", "es-AR", "fil", "fr", "ja", "ru", "sr", "sr-Cyrl", "sr-Latn", "th", "th-u-nu-thai", "tr", "und"].binary_search_by(|k| $req.locale.strict_cmp(k.as_bytes()).reverse()).ok().map(|i| unsafe {
            ...
            *[&AR, &AR, &AR, &AR, &BN, &BN, &CCP, &CCP, &EN, &EN, &EN, &ES, &ES_AR, &FIL, &FR, &JA, &RU, &SR, &SR, &SR_LATN, &TH, &TH, &TR, &CCP].get_unchecked(i)
        })
    };
}
    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_compactdecimal::provider::LongCompactDecimalFormatDataV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_compactdecimal::provider::LongCompactDecimalFormatDataV1Marker>, icu_provider::DataError> {
                let lookup = lookup_compactdecimal_long_v1!(req);
                lookup.map(icu_provider::prelude::zerofrom::ZeroFrom::zero_from).map(icu_provider::DataPayload::from_owned).map(|payload| icu_provider::DataResponse { metadata: Default::default(), payload: Some(payload) }).ok_or_else(|| icu_provider::DataErrorKind::MissingLocale.with_req(<icu_compactdecimal::provider::LongCompactDecimalFormatDataV1Marker as icu_provider::KeyedDataMarker>::KEY, req))
            }
        }
    };
}
