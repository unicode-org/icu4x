...
#[doc(hidden)]
#[macro_export]
macro_rules! __lookup_props_radical_v1 {
    ($ locale : expr) => {{
        static SINGLETON: <icu_properties::provider::RadicalV1Marker as icu_provider::DataMarker>::Yokeable = singleton_props_radical_v1!();
        if $locale.is_empty() {
            Ok(&SINGLETON)
        } else {
    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_properties::provider::RadicalV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_properties::provider::RadicalV1Marker>, icu_provider::DataError> {
                match lookup_props_radical_v1!(req.locale) {
