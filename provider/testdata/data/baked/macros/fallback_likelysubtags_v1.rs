...
#[doc(hidden)]
#[macro_export]
macro_rules! __singleton_fallback_likelysubtags_v1 {
    () => {
...
#[doc(hidden)]
#[macro_export]
macro_rules! __lookup_fallback_likelysubtags_v1 {
    ($ locale : expr) => {{
        static SINGLETON: <icu_provider_adapters::fallback::provider::LocaleFallbackLikelySubtagsV1Marker as icu_provider::DataMarker>::Yokeable = singleton_fallback_likelysubtags_v1!();
        if $locale.is_empty() {
            Ok(&SINGLETON)
        } else {
...
/// Implement [`DataProvider<LocaleFallbackLikelySubtagsV1Marker>`](icu_provider::DataProvider) on the given struct using the data
    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_provider_adapters::fallback::provider::LocaleFallbackLikelySubtagsV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_provider_adapters::fallback::provider::LocaleFallbackLikelySubtagsV1Marker>, icu_provider::DataError> {
                match lookup_fallback_likelysubtags_v1!(req.locale) {
