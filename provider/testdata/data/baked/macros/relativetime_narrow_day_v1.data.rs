    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_relativetime::provider::NarrowDayRelativeTimeFormatDataV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_relativetime::provider::NarrowDayRelativeTimeFormatDataV1Marker>, icu_provider::DataError> {
                ...
                static VALUES: [&<icu_relativetime::provider::NarrowDayRelativeTimeFormatDataV1Marker as icu_provider::DataMarker>::Yokeable; 19usize] = [&AR, &AR, &BN, &CCP, &EN, &EN_001, &EN_001, &ES, &ES_AR, &FIL, &FR, &JA, &RU, &SR, &SR, &SR_LATN, &TH, &TR, &UND];
                ...
                if let Ok(payload) = KEYS.binary_search_by(|k| req.locale.strict_cmp(k.as_bytes()).reverse()).map(|i| *unsafe { VALUES.get_unchecked(i) }) {
                    ...
                    Err(icu_provider::DataErrorKind::MissingLocale.with_req(<icu_relativetime::provider::NarrowDayRelativeTimeFormatDataV1Marker as icu_provider::KeyedDataMarker>::KEY, req))
                }
            }
        }
    };
