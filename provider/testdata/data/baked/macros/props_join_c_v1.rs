...
#[doc(hidden)]
#[macro_export]
macro_rules! __singleton_props_join_c_v1 {
    () => {
...
#[doc(hidden)]
#[macro_export]
macro_rules! __lookup_props_join_c_v1 {
    ($ locale : expr) => {{
        static SINGLETON: <icu_properties::provider::JoinControlV1Marker as icu_provider::DataMarker>::Yokeable = singleton_props_join_c_v1!();
        if $locale.is_empty() {
            Ok(&SINGLETON)
        } else {
    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_properties::provider::JoinControlV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_properties::provider::JoinControlV1Marker>, icu_provider::DataError> {
                match lookup_props_join_c_v1!(req.locale) {
