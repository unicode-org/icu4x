...
#[doc(hidden)]
#[macro_export]
macro_rules! __singleton_propnames_from_wb_v1 {
    () => {
...
#[doc(hidden)]
#[macro_export]
macro_rules! __lookup_propnames_from_wb_v1 {
    ($ req : expr) => {
    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_properties::provider::WordBreakNameToValueV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_properties::provider::WordBreakNameToValueV1Marker>, icu_provider::DataError> {
                let lookup = lookup_propnames_from_wb_v1!(req);
                lookup.map(icu_provider::prelude::zerofrom::ZeroFrom::zero_from).map(icu_provider::DataPayload::from_owned).map(|payload| icu_provider::DataResponse { metadata: Default::default(), payload: Some(payload) }).ok_or_else(|| icu_provider::DataErrorKind::MissingLocale.with_req(<icu_properties::provider::WordBreakNameToValueV1Marker as icu_provider::KeyedDataMarker>::KEY, req))
            }
        }
    };
}
