...
#[doc(hidden)]
#[macro_export]
macro_rules! __lookup_datetime_ethiopic_datesymbols_v1 {
    ($ req : expr) => {
    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_datetime::provider::calendar::EthiopianDateSymbolsV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_datetime::provider::calendar::EthiopianDateSymbolsV1Marker>, icu_provider::DataError> {
                let lookup = lookup_datetime_ethiopic_datesymbols_v1!(req);
                lookup.map(icu_provider::prelude::zerofrom::ZeroFrom::zero_from).map(icu_provider::DataPayload::from_owned).map(|payload| icu_provider::DataResponse { metadata: Default::default(), payload: Some(payload) }).ok_or_else(|| icu_provider::DataErrorKind::MissingLocale.with_req(<icu_datetime::provider::calendar::EthiopianDateSymbolsV1Marker as icu_provider::KeyedDataMarker>::KEY, req))
            }
        }
    };
}
