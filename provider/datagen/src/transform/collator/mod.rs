...
fn open_reader(path: &Path) -> Result<BufReader<File>, DataError> {
...
fn read_path_to_string(path: &Path) -> Result<String, DataError> {
...
fn get_dir_contents(root: &Path) -> Result<Vec<PathBuf>, DataError> {
    let mut result = vec![];
