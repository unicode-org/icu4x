    ...
    ($marker:ident) => {
        impl DataProvider<$marker> for crate::DatagenProvider {
            fn load(&self, req: DataRequest) -> Result<DataResponse<$marker>, DataError> {
                ...
                let (wide, short, narrow) = if <$marker>::KEY == AndListV1Marker::KEY {
                    (&data.standard, &data.standard_short, &data.standard_narrow)
                } else if <$marker>::KEY == OrListV1Marker::KEY {
                    (&data.or, &data.or_short, &data.or_narrow)
                } else if <$marker>::KEY == UnitListV1Marker::KEY {
                ...
                if langid.language == language!("es") {
                    if <$marker>::KEY == AndListV1Marker::KEY || <$marker>::KEY == UnitListV1Marker::KEY {
                        lazy_static! {
                ...
                if langid.language == language!("he") {
                    ...
                    let non_hebrew = StringMatcher::new(&format!(
                        "[^{}]",
                        icu_properties::maps::get_script(self)
