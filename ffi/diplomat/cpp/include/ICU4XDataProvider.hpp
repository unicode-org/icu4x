...
class ICU4XDataProvider {
 public:
   ...
   */
  static ICU4XCreateDataProviderResult create_blob_from_slice(const std::span<uint8_t> blob);
...
inline ICU4XCreateDataProviderResult ICU4XDataProvider::create_blob_from_slice(const std::span<uint8_t> blob) {
  capi::ICU4XCreateDataProviderResult diplomat_raw_struct_out_value = capi::ICU4XDataProvider_create_blob_from_slice(blob.data(), blob.size());
  auto diplomat_optional_raw_out_value_provider = diplomat_raw_struct_out_value.provider;
  std::optional<ICU4XDataProvider> diplomat_optional_out_value_provider;
  if (diplomat_optional_raw_out_value_provider != nullptr) {
    diplomat_optional_out_value_provider = ICU4XDataProvider(diplomat_optional_raw_out_value_provider);
