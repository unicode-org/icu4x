...
internal interface WordSegmenterLib: Library {
    ...
    fun icu4x_WordSegmenter_create_non_complex_mv1(): Pointer
    fun icu4x_WordSegmenter_create_non_complex_with_content_locale_mv1(locale: Pointer): ResultPointerInt
    fun icu4x_WordSegmenter_create_non_complex_with_content_locale_and_provider_mv1(provider: Pointer, locale: Pointer): ResultPointerInt
...
class WordSegmenter internal constructor (
    ...
    companion object {
        ...
        */
        fun createNonComplex(): WordSegmenter {
            
            val returnVal = lib.icu4x_WordSegmenter_create_non_complex_mv1();
            val selfEdges: List<Any> = listOf()
            val handle = returnVal 
            val returnOpaque = WordSegmenter(handle, selfEdges)
            CLEANER.register(returnOpaque, WordSegmenter.WordSegmenterCleaner(handle, WordSegmenter.lib));
        ...
        */
        fun createNonComplexWithContentLocale(locale: Locale): Result<WordSegmenter> {
            
            val returnVal = lib.icu4x_WordSegmenter_create_non_complex_with_content_locale_mv1(locale.handle);
            if (returnVal.isOk == 1.toByte()) {
                val selfEdges: List<Any> = listOf()
                val handle = returnVal.union.ok 
                val returnOpaque = WordSegmenter(handle, selfEdges)
        ...
        *See the [Rust documentation for `try_new_non_complex`](https://docs.rs/icu/2.1.1/icu/segmenter/struct.WordSegmenter.html#method.try_new_non_complex) for more information.
        */
        fun createNonComplexWithContentLocaleAndProvider(provider: DataProvider, locale: Locale): Result<WordSegmenter> {
            
            val returnVal = lib.icu4x_WordSegmenter_create_non_complex_with_content_locale_and_provider_mv1(provider.handle, locale.handle);
            if (returnVal.isOk == 1.toByte()) {
                val selfEdges: List<Any> = listOf()
                val handle = returnVal.union.ok 
                val returnOpaque = WordSegmenter(handle, selfEdges)
