 ...
 * An ICU4X TimeZoneFormatter object capable of formatting an {@link CustomTimeZone `CustomTimeZone`} type (and others) as a string
...
export class TimeZoneFormatter {
   ...
   * Creates a new {@link TimeZoneFormatter `TimeZoneFormatter`} from locale data.
   ...
   */
  static create_with_localized_gmt_fallback(provider: DataProvider, locale: Locale): TimeZoneFormatter | never;
   ...
   * @throws {@link FFIError}<{@link Error}>
   */
  static create_with_iso_8601_fallback(provider: DataProvider, locale: Locale, options: IsoTimeZoneOptions): TimeZoneFormatter | never;
   ...
   * @throws {@link FFIError}<{@link Error}>
   */
  load_generic_non_location_long(provider: DataProvider): void | never;
   ...
   * @throws {@link FFIError}<{@link Error}>
   */
  load_generic_non_location_short(provider: DataProvider): void | never;
   ...
   * @throws {@link FFIError}<{@link Error}>
   */
  load_specific_non_location_long(provider: DataProvider): void | never;
   ...
   * @throws {@link FFIError}<{@link Error}>
   */
  load_specific_non_location_short(provider: DataProvider): void | never;
   ...
   * @throws {@link FFIError}<{@link Error}>
   */
  load_generic_location_format(provider: DataProvider): void | never;
   ...
   * @throws {@link FFIError}<{@link Error}>
   */
  load_iso_8601_format(options: IsoTimeZoneOptions): void | never;

  /**

   * Formats a {@link CustomTimeZone `CustomTimeZone`} to a string.
   ...
   */
  format_custom_time_zone(value: CustomTimeZone): string;

  /**

   * Formats a {@link CustomTimeZone `CustomTimeZone`} to a string, performing no fallback
   ...
   * @throws {@link FFIError}<{@link Error}>
   */
  format_custom_time_zone_no_fallback(value: CustomTimeZone): string | never;
}
