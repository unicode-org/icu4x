...
const PluralOperands_box_destroy_registry = new FinalizationRegistry(underlying => {
...
export class PluralOperands {
  ...
  constructor(underlying, owned, edges) {
    ...
    if (owned) {
      PluralOperands_box_destroy_registry.register(this, underlying);
    }
  }
  ...
  static create_from_string(arg_s) {
    ...
    const diplomat_out = (() => {
      ...
      if (is_ok) {
        const ok_value = new PluralOperands(diplomatRuntime.ptrRead(wasm, diplomat_receive_buffer), true, []);
  ...
  static create_from_fixed_decimal(arg_x) {
    return new PluralOperands(wasm.ICU4XPluralOperands_create_from_fixed_decimal(arg_x.underlying), true, []);
  }
}
