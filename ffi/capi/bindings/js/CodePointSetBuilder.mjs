...
const CodePointSetBuilder_box_destroy_registry = new FinalizationRegistry(underlying => {
...
export class CodePointSetBuilder {
  ...
  constructor(underlying, owned, edges) {
    ...
    if (owned) {
      CodePointSetBuilder_box_destroy_registry.register(this, underlying);
    }
  }

  static create() {
    return new CodePointSetBuilder(wasm.ICU4XCodePointSetBuilder_create(), true, []);
  }

  build() {
    return new CodePointSetData(wasm.ICU4XCodePointSetBuilder_build(this.underlying), true, []);
  }
