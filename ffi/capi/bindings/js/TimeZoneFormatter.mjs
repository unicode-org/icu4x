...
const TimeZoneFormatter_box_destroy_registry = new FinalizationRegistry(underlying => {
...
export class TimeZoneFormatter {
  ...
  constructor(underlying, owned, edges) {
    ...
    if (owned) {
      TimeZoneFormatter_box_destroy_registry.register(this, underlying);
    }
  }
  ...
  static create_with_localized_gmt_fallback(arg_provider, arg_locale) {
    return (() => {
      ...
      if (is_ok) {
        const ok_value = new TimeZoneFormatter(diplomatRuntime.ptrRead(wasm, diplomat_receive_buffer), true, []);
  ...
  load_iso_8601_format(arg_options) {
    ...
    return (() => {
      const diplomat_receive_buffer = wasm.diplomat_alloc(5, 4);
      wasm.ICU4XTimeZoneFormatter_load_iso_8601_format(diplomat_receive_buffer, this.underlying, IsoTimeZoneFormat_js_to_rust[field_format_arg_options], IsoTimeZoneMinuteDisplay_js_to_rust[field_minutes_arg_options], IsoTimeZoneSecondDisplay_js_to_rust[field_seconds_arg_options]);
      const is_ok = diplomatRuntime.resultFlag(wasm, diplomat_receive_buffer, 4);
      if (is_ok) {
        const ok_value = {};
        wasm.diplomat_free(diplomat_receive_buffer, 5, 4);
