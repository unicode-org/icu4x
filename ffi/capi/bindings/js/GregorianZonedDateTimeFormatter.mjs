...
const GregorianZonedDateTimeFormatter_box_destroy_registry = new FinalizationRegistry(underlying => {
...
export class GregorianZonedDateTimeFormatter {
  ...
  constructor(underlying, owned, edges) {
    ...
    if (owned) {
      GregorianZonedDateTimeFormatter_box_destroy_registry.register(this, underlying);
    }
  }
  ...
  static create_with_lengths(arg_provider, arg_locale, arg_date_length, arg_time_length) {
    return (() => {
      ...
      wasm.ICU4XGregorianZonedDateTimeFormatter_create_with_lengths(diplomat_receive_buffer, arg_provider.underlying, arg_locale.underlying, DateLength_js_to_rust[arg_date_length], TimeLength_js_to_rust[arg_time_length]);
      const is_ok = diplomatRuntime.resultFlag(wasm, diplomat_receive_buffer, 4);
      if (is_ok) {
        const ok_value = new GregorianZonedDateTimeFormatter(diplomatRuntime.ptrRead(wasm, diplomat_receive_buffer), true, []);
