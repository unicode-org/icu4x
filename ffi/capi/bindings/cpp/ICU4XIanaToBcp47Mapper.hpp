...
class ICU4XIanaToBcp47Mapper {
 public:
   ...
   */
  template<typename W> diplomat::result<std::monostate, ICU4XError> get_to_write(const std::string_view value, W& write) const;
...
template<typename W> inline diplomat::result<std::monostate, ICU4XError> ICU4XIanaToBcp47Mapper::get_to_write(const std::string_view value, W& write) const {
...
inline diplomat::result<std::string, ICU4XError> ICU4XIanaToBcp47Mapper::get(const std::string_view value) const {
  ...
  auto diplomat_result_raw_out_value = capi::ICU4XIanaToBcp47Mapper_get(this->inner.get(), value.data(), value.size(), &diplomat_write_out);
  diplomat::result<std::monostate, ICU4XError> diplomat_result_out_value;
  if (diplomat_result_raw_out_value.is_ok) {
    diplomat_result_out_value = diplomat::Ok<std::monostate>(std::monostate());
  } else {
