...
namespace capi {
    ...
    typedef struct icu4x_DateFormatter_create_with_length_mv1_result {union {icu4x::capi::DateFormatter* ok; icu4x::capi::PatternLoadError err;}; bool is_ok;} icu4x_DateFormatter_create_with_length_mv1_result;
...
inline diplomat::result<std::unique_ptr<icu4x::DateFormatter>, icu4x::PatternLoadError> icu4x::DateFormatter::create_with_length(const icu4x::DataProvider& provider, const icu4x::Locale& locale, icu4x::DateTimeLength length) {
  auto result = icu4x::capi::icu4x_DateFormatter_create_with_length_mv1(provider.AsFFI(),
    locale.AsFFI(),
    length.AsFFI());
  return result.is_ok ? diplomat::result<std::unique_ptr<icu4x::DateFormatter>, icu4x::PatternLoadError>(diplomat::Ok<std::unique_ptr<icu4x::DateFormatter>>(std::unique_ptr<icu4x::DateFormatter>(icu4x::DateFormatter::FromFFI(result.ok)))) : diplomat::result<std::unique_ptr<icu4x::DateFormatter>, icu4x::PatternLoadError>(diplomat::Err<icu4x::PatternLoadError>(icu4x::PatternLoadError::FromFFI(result.err)));
}
