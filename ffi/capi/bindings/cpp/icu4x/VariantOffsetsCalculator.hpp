...
inline std::optional<icu4x::VariantOffsets> icu4x::VariantOffsetsCalculator::compute_offsets_from_time_zone(const icu4x::TimeZone& time_zone, const icu4x::IsoDate& local_date, const icu4x::Time& local_time) const {
  auto result = icu4x::capi::icu4x_VariantOffsetsCalculator_compute_offsets_from_time_zone_mv1(this->AsFFI(),
    time_zone.AsFFI(),
    local_date.AsFFI(),
    local_time.AsFFI());
