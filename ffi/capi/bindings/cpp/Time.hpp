...
namespace capi {
    ...
    typedef struct icu4x_Time_from_string_mv1_result {union {diplomat::capi::Time* ok; diplomat::capi::CalendarFromStrError err;}; bool is_ok;} icu4x_Time_from_string_mv1_result;
...
inline diplomat::result<std::unique_ptr<Time>, CalendarFromStrError> Time::from_string(std::string_view v) {
  auto result = diplomat::capi::icu4x_Time_from_string_mv1(v.data(),
    v.size());
  return result.is_ok ? diplomat::result<std::unique_ptr<Time>, CalendarFromStrError>(diplomat::Ok<std::unique_ptr<Time>>(std::unique_ptr<Time>(Time::FromFFI(result.ok)))) : diplomat::result<std::unique_ptr<Time>, CalendarFromStrError>(diplomat::Err<CalendarFromStrError>(CalendarFromStrError::FromFFI(result.err)));
}
