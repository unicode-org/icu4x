...
let termini = Object.assign({
    ...
    "ZonedDateTimeFormatter.formatSameCalendar": {
        func: (selfLocaleName, selfFormatterLocaleName, selfFormatterLength, selfFormatterTimePrecision, selfFormatterAlignment, selfFormatterYearStyle, dateIsoYear, dateIsoMonth, dateIsoDay, dateCalendarKind, timeHour, timeMinute, timeSecond, timeSubsecond, zoneIdId, zoneOffsetOffset, zoneVariant) => icu.ZonedDateTimeFormatter.createSpecificLong(icu.Locale.fromString(selfLocaleName), icu.DateTimeFormatter.createYmdt(icu.Locale.fromString(selfFormatterLocaleName), selfFormatterLength, selfFormatterTimePrecision, selfFormatterAlignment, selfFormatterYearStyle)).formatSameCalendar(icu.Date.fromIsoInCalendar(dateIsoYear, dateIsoMonth, dateIsoDay, new icu.Calendar(dateCalendarKind)), new icu.Time(timeHour, timeMinute, timeSecond, timeSubsecond), new icu.TimeZoneInfo(icu.TimeZone.createFromBcp47(zoneIdId), icu.UtcOffset.fromString(zoneOffsetOffset), zoneVariant)),
        // For avoiding webpacking minifying issues:
        funcName: "ZonedDateTimeFormatter.formatSameCalendar",
        expr: (selfLocaleName, selfFormatterLocaleName, selfFormatterLength, selfFormatterTimePrecision, selfFormatterAlignment, selfFormatterYearStyle, dateIsoYear, dateIsoMonth, dateIsoDay, dateCalendarKind, timeHour, timeMinute, timeSecond, timeSubsecond, zoneIdId, zoneOffsetOffset, zoneVariant) => "icu.ZonedDateTimeFormatter.createSpecificLong(icu.Locale.fromString(selfLocaleName), icu.DateTimeFormatter.createYmdt(icu.Locale.fromString(selfFormatterLocaleName), selfFormatterLength, selfFormatterTimePrecision, selfFormatterAlignment, selfFormatterYearStyle)).formatSameCalendar(icu.Date.fromIsoInCalendar(dateIsoYear, dateIsoMonth, dateIsoDay, new icu.Calendar(dateCalendarKind)), new icu.Time(timeHour, timeMinute, timeSecond, timeSubsecond), new icu.TimeZoneInfo(icu.TimeZone.createFromBcp47(zoneIdId), icu.UtcOffset.fromString(zoneOffsetOffset), zoneVariant))".replace(/([\( ])selfLocaleName([,\) \n])/, '$1' + selfLocaleName + '$2').replace(/([\( ])selfFormatterLocaleName([,\) \n])/, '$1' + selfFormatterLocaleName + '$2').replace(/([\( ])selfFormatterLength([,\) \n])/, '$1' + selfFormatterLength + '$2').replace(/([\( ])selfFormatterTimePrecision([,\) \n])/, '$1' + selfFormatterTimePrecision + '$2').replace(/([\( ])selfFormatterAlignment([,\) \n])/, '$1' + selfFormatterAlignment + '$2').replace(/([\( ])selfFormatterYearStyle([,\) \n])/, '$1' + selfFormatterYearStyle + '$2').replace(/([\( ])dateIsoYear([,\) \n])/, '$1' + dateIsoYear + '$2').replace(/([\( ])dateIsoMonth([,\) \n])/, '$1' + dateIsoMonth + '$2').replace(/([\( ])dateIsoDay([,\) \n])/, '$1' + dateIsoDay + '$2').replace(/([\( ])dateCalendarKind([,\) \n])/, '$1' + dateCalendarKind + '$2').replace(/([\( ])timeHour([,\) \n])/, '$1' + timeHour + '$2').replace(/([\( ])timeMinute([,\) \n])/, '$1' + timeMinute + '$2').replace(/([\( ])timeSecond([,\) \n])/, '$1' + timeSecond + '$2').replace(/([\( ])timeSubsecond([,\) \n])/, '$1' + timeSubsecond + '$2').replace(/([\( ])zoneIdId([,\) \n])/, '$1' + zoneIdId + '$2').replace(/([\( ])zoneOffsetOffset([,\) \n])/, '$1' + zoneOffsetOffset + '$2').replace(/([\( ])zoneVariant([,\) \n])/, '$1' + zoneVariant + '$2'),
        parameters: [
