...
fn main() -> eyre::Result<()> {
    ...
    extract(
        ...
        .with_context(|| "Failed to download LSTM ZIP".to_owned())?,
        TZDB_GLOB.iter().copied().map(|p| format!("tz-{}/{p}", SourceDataProvider::LATEST_TESTED_TZDB_TAG)).collect(),
        out_root.join("tests/data"),
        &mut Default::default(),
    )?;
    std::fs::rename(out_root.join(format!("tests/data/tz-{}", SourceDataProvider::LATEST_TESTED_TZDB_TAG)), out_root.join("tests/data/tzdb")).unwrap();
    ...
    let tzdb_data: String = TZDB_GLOB
        ...
        .map(|path| {
            let path = path.replace('\\', "/");
            format!(r#"("tz-tag/{path}", include_bytes!("../../tests/data/tzdb/{path}").as_slice())"#)
        })
        .collect::<Vec<_>>()
        .join(",\n                        ");
