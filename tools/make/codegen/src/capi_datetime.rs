...
#[derive(Template)]
#[template(path = "range_formatter.rs.jinja")]
struct RangeFormatterTemplate {
    flavor: FormatterFlavor,
    variants: Vec<DateTimeFormatterVariant>,
}
...
pub fn main() {
    ...
    let date_range_formatter_template = RangeFormatterTemplate {
        flavor: FormatterFlavor::Date,
        variants: date_formatter_template.variants,
    };
    let time_range_formatter_template = RangeFormatterTemplate {
        flavor: FormatterFlavor::Time,
        variants: time_formatter_template.variants,
    };
    let datetime_range_formatter_template = RangeFormatterTemplate {
        flavor: FormatterFlavor::DateTime,
        variants: datetime_formatter_template.variants,
    };

    {
        let mut path_buf = path_buf.clone();
        path_buf.push("date_range_formatter.rs");
        let mut file = File::create(&path_buf).unwrap();
        use std::io::Write;
        writeln!(&mut file, "{date_range_formatter_template}").unwrap();
    }

    {
        let mut path_buf = path_buf.clone();
        path_buf.push("time_range_formatter.rs");
        let mut file = File::create(&path_buf).unwrap();
        use std::io::Write;
        writeln!(&mut file, "{time_range_formatter_template}").unwrap();
    }

    {
        let mut path_buf = path_buf.clone();
        path_buf.push("date_time_range_formatter.rs");
        let mut file = File::create(&path_buf).unwrap();
        use std::io::Write;
        writeln!(&mut file, "{datetime_range_formatter_template}").unwrap();
    }
}
