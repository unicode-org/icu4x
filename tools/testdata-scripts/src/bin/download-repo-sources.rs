...
include!("../../../../provider/bikeshed/tests/locales.rs.data");
...
fn main() -> eyre::Result<()> {
    ...
    let out_root =
        std::path::Path::new(std::env!("CARGO_MANIFEST_DIR")).join("../../provider/bikeshed");
...
use crate::{{AbstractFs, CldrCache, DatagenProvider, SerdeCache}};
