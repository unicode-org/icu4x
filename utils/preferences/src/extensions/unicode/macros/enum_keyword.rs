...
macro_rules! enum_keyword_inner {
    ...
    ($name:ident, $variant:ident, $s:ident, $v2:ident) => {{
        if $s.len() < 1 {
            ...
            $name::$variant(Some($v2::try_from($s)?))
        }
    }};
}
...
macro_rules! enum_keyword {
    ($name:ident {
        ...
        pub enum $name {
            ...
            Custom(icu_locale_core::extensions::unicode::Value)
        }
    ...
    ($name:ident {
        $($key:expr => $variant:ident $(($v2:ident) {$($subk:expr => $subv:ident),*})?),* $(,)?
    }, $ext_key:literal, custom true) => {
        crate::enum_keyword!($name {$($key => $variant $(($v2) {$($subk => $subv),*})?),*}, custom true);
...
mod tests {
    ...
    fn enum_keywords_custom_test() {
        ...
        let dk: DummyKeyword = v.clone().try_into().unwrap();
        assert_eq!(dk, DummyKeyword::Custom(unicode::value!("foo")));
        assert_eq!(unicode::Value::from(dk), v);
    }

    #[test]
    fn enum_keywords_nested_custom_test() {
        ...
        assert_eq!(
            dk,
            DummyKeyword::Sub(Some(DummySubKeyword::Custom(unicode::value!("foo"))))
        );
        assert_eq!(unicode::Value::from(dk), v);
    }
}
