...
use crate::{codepointset::CodePointSet, utils::deconstruct_range};
...
impl CodePointSetBuilder {
    ...
    ///
    /// ```
    /// use icu_codepointset::{CodePointSetBuilder, CodePointSet};
    /// let mut builder = CodePointSetBuilder::new();
    ...
    /// use icu_codepointset::{CodePointSetBuilder, CodePointSet};
    /// let mut builder = CodePointSetBuilder::new();
    /// let set = CodePointSet::from_inversion_list_slice(&[0x0, 0x41, 0x46, (std::char::MAX as u32) + 1]).unwrap();
    /// builder.add_set(&set);
    /// builder.complement();
    /// let check = builder.build();
    /// assert_eq!(check.iter_chars().next(), Some('A'));
