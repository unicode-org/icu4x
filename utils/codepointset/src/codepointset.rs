...
/// A membership wrapper for [`CodePointSet`].
///
/// Provides exposure to membership functions and constructors from serialized [`CodePointSets`](CodePointSet)
...
impl<'de: 'a, 'a> serde::Deserialize<'de> for CodePointSet<'a> {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        CodePointSet::from_inversion_list(parsed_inv_list).map_err(|e| {
            Error::custom(format!(
                "Cannot deserialize invalid inversion list for CodePointSet: {:?}",
                e
            ))
        })
    }
...
#[cfg(any(feature = "serde", test))]
impl<'data> serde::Serialize for CodePointSet<'data> {
...
impl<'data> CodePointSet<'data> {
    /// Returns a new [`CodePointSet`] from an [inversion list](https://en.wikipedia.org/wiki/Inversion_list)
    ...
    /// use icu_codepointset::CodePointSetError;
    /// use zerovec::ZeroVec;
    /// let valid = [0x0, 0x10000];
    /// let inv_list: ZeroVec<u32> = ZeroVec::from_slice(&valid);
    /// let result = CodePointSet::from_inversion_list(inv_list);
    /// assert!(matches!(result, CodePointSet));
    ///
    ...
    pub fn from_inversion_list(inv_list: ZeroVec<'data, u32>) -> Result<Self, CodePointSetError> {
        if is_valid_zv(&inv_list) {
            ...
            Err(CodePointSetError::InvalidSet(inv_list.to_vec()))
        }
    }

    /// Returns a new [`CodePointSet`] by borrowing an [inversion list](https://en.wikipedia.org/wiki/Inversion_list)
    ...
    /// use zerovec::ZeroVec;
    /// let valid = [0x0, 0x10000];
    /// let result = CodePointSet::from_inversion_list_slice(&valid);
    /// assert!(matches!(result, CodePointSet));
    ///
    /// let invalid: Vec<u32> = vec![0x0, 0x80, 0x3];
    /// let result = CodePointSet::from_inversion_list_slice(&invalid);
    /// assert!(matches!(result, Err(CodePointSetError::InvalidSet(_))));
    /// if let Err(CodePointSetError::InvalidSet(actual)) = result {
    ///     assert_eq!(&invalid, &actual);
    /// }
    /// ```
    pub fn from_inversion_list_slice(inv_list: &'data [u32]) -> Result<Self, CodePointSetError> {
        let inv_list_zv: ZeroVec<u32> = ZeroVec::from_slice(inv_list);
        CodePointSet::from_inversion_list(inv_list_zv)
    }

    /// Returns a new, fully-owned [`CodePointSet`] by cloning an [inversion list](https://en.wikipedia.org/wiki/Inversion_list)
    /// represented as a slice of [`u32`] code points.
    ///
    /// The inversion list must be of even length, sorted ascending non-overlapping,
    /// and within the bounds of `0x0 -> 0x10FFFF` inclusive, and end points being exclusive.
    ...
    /// Returns [`CodePointSet`] spanning entire Unicode range
    ///
    ...
    /// assert_eq!(CodePointSet::all().get_inversion_list(), expected);
    /// assert_eq!(
    ///     CodePointSet::all().size(),
    ...
    /// Returns [`CodePointSet`] spanning BMP range
    ///
    ...
    /// assert_eq!(CodePointSet::bmp().get_inversion_list(), expected);
    /// assert_eq!(
    ///     CodePointSet::bmp().size(),
    ///     (expected[1] - expected[0]) as usize
    /// );
    /// ```
    pub fn bmp() -> Self {
    ...
    /// Yields an [`Iterator`] going through the character set in the [`CodePointSet`]
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// let example = CodePointSet::from_inversion_list_slice(&example_list).unwrap();
    ...
    /// behavior of ranges, ex: `CodePointSet::contains(UChar32 start, UChar32 end)`.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// Returns the number of ranges contained in this [`CodePointSet`]
    pub fn get_range_count(&self) -> usize {
        self.inv_list.len() / 2
    }

    /// Returns a specific range contained in this [`CodePointSet`] by index.
    ...
    /// Returns the number of elements of the [`CodePointSet`]
    ...
    /// Returns whether or not the [`CodePointSet`] is empty
    ...
    /// Checks to see the query is in the [`CodePointSet`]
    ///
    ...
    /// Checks to see the unsigned int is in the [`CodePointSet::all()`](CodePointSet::all())
    ///
    ...
    /// Checks to see if the range is in the [`CodePointSet`]
    ///
    /// Runs a binary search in `O(log(n))` where `n` is the number of start and end points
    ...
    /// The ICU4C/J `CodePointSet::contains(UChar32 start, UChar32 end)` method
    ...
    /// Check if the calling [`CodePointSet`] contains all the characters of the given [`CodePointSet`]
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// let example = CodePointSet::from_inversion_list_slice(&example_list).unwrap();
    /// let a_to_d = CodePointSet::from_inversion_list_slice(&[0x41, 0x45]).unwrap();
    /// let f_to_t = CodePointSet::from_inversion_list_slice(&[0x46, 0x55]).unwrap();
    /// let r_to_x = CodePointSet::from_inversion_list_slice(&[0x52, 0x58]).unwrap();
...
#[cfg(test)]
mod tests {
    use super::{CodePointSet, CodePointSetError};
    ...
    fn test_unicodeset_try_from_vec_error() {
        ...
        assert!(matches!(set, Err(CodePointSetError::InvalidSet(_))));
        if let Err(CodePointSetError::InvalidSet(actual)) = set {
    ...
    fn test_unicodeset_contains_set_u() {
        ...
        let s = CodePointSet::from_inversion_list_slice(&inside).unwrap();
    ...
    fn test_unicodeset_contains_set_u_false() {
        ...
        let s = CodePointSet::from_inversion_list_slice(&outside).unwrap();
        assert!(!u.contains_set(&s));
    }
    ...
    #[test]
    fn test_codepointset_to_inv_list() {
        ...
        let s: CodePointSet = CodePointSet::from_inversion_list_slice(&inv_list_clone).unwrap();
    ...
    fn test_serde_serialize() {
        ...
        let codepointset = CodePointSet::from_inversion_list_slice(&inv_list).unwrap();
        let json_str = serde_json::to_string(&codepointset).unwrap();
    ...
    fn test_serde_deserialize() {
        ...
        let exp_codepointset = CodePointSet::from_inversion_list_slice(&exp_inv_list).unwrap();
        let act_codepointset: CodePointSet = serde_json::from_str(inv_list_str).unwrap();
        assert_eq!(act_codepointset, exp_codepointset);
    }

    #[test]
    fn test_serde_deserialize_invalid() {
        let inv_list_str = "[65,70,98775,85]";
        let act_result: Result<CodePointSet, serde_json::Error> =
    ...
    fn test_serde_with_postcard_roundtrip() -> Result<(), postcard::Error> {
        let set = CodePointSet::bmp();
        let set_serialized: Vec<u8> = postcard::to_allocvec(&set).unwrap();
        let set_deserialized: CodePointSet = postcard::from_bytes::<CodePointSet>(&set_serialized)?;
