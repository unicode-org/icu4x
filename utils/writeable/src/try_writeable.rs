...
pub trait TryWriteable {
    ...
    fn try_writeable_length_hint(&self) -> LengthHint {
        LengthHint::undefined()
    }

    fn try_write_to_string(&self) -> Result<Cow<str>, Self::Error> {
        let hint = self.try_writeable_length_hint();
        if hint.is_zero() {
    ...
    fn try_write_cmp_bytes(&self, other: &[u8]) -> core::cmp::Ordering {
...
macro_rules! assert_try_writeable_eq {
    ($actual_writeable:expr, $expected_str:expr, $expected_result:expr $(,)?) => {
        $crate::assert_writeable_eq!($actual_writeable, $expected_result, $expected_str, "");
    };
    ($actual_writeable:expr, $expected_str:expr, $expected_result:expr, $($arg:tt)+) => {{
        ...
        let actual_result = match $crate::Writeable::try_write_to_string(actual_writeable) {
            Ok(s) => {
