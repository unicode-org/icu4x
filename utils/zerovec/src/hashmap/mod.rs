...
#[inline]
fn compute_hash<A, K: Hash + ?Sized>(k: &A, m: usize) -> (usize, u32, u32)
where
    A: Borrow<K>,
{
    let mut hasher = T1haHasher::with_seed(SEED);
    k.borrow().hash(&mut hasher);
...
/// Compute displacements for the given `keys`, which split the keys into distinct slots by a
...
///
/// Returns
fn compute_displacements<A, K: Hash + PartialEq + ?Sized>(
    keys: impl ExactSizeIterator<Item = A>,
        ...
        'd0: for d0 in 0..len as u32 {
            'd1: for d1 in 0..len as u32 {
                ...
                for ((_, f0, f1), _) in buckets {
                    let displacement_idx = compute_displacement((*f0, *f1), (d0, d1), len as u32);
    ...
    V: ZeroMapKV<'a> + ?Sized,
{
        ...
        I: Iterator<Item = (A, B)>,
    {
        ...
        let (displacements, mut reverse_mapping) = compute_displacements(keys_vec.into_iter());
