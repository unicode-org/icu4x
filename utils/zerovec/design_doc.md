...
One way to handle this is annotate the original enum with, as well as create an identical enum with `#[repr(C, u8)]`. Then transforming the fields of the identical enum into their `AsULE` variants. Then on the new enum the `as_byte_slice` method would transmute itself into a size based on the size of it's current variant plus one for the discriminant, This is somewhat complicated but actually relatively simple to implement since it can use `#[make_ule]` and `#[make_varule]`.
