...
fn test_vec_validation() {
    ...
    // Should succeed
    ZeroSlice::<MyTuple>::parse_byte_slice(zv.as_bytes()).unwrap();
    ...
    // Should fail
    let err = ZeroSlice::<MyTuple>::parse_byte_slice(&invalid_bytes).unwrap_err();
    assert!(matches!(err, ZeroVecError::ParseError { .. }));
}
...
fn test_make_ule_vec_validation() {
    ...
    // Should succeed
    ZeroSlice::<MyStruct>::parse_byte_slice(zv.as_bytes()).unwrap();
    ...
    invalid_bytes[9] = 0xff;
    // Should fail
    let err = ZeroSlice::<MyStruct>::parse_byte_slice(&invalid_bytes).unwrap_err();
    assert!(matches!(err, ZeroVecError::ParseError { .. }));
}
