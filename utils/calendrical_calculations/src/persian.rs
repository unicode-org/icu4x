...
lazy_static::lazy_static! {
    static ref NON_LEAP_CORRECTION: HashSet<i32> = HashSet::from([
...
const MIN_NON_LEAP_CORRECTION: i32 = 1502;
...
pub fn fixed_from_fast_persian(year: i32, month: u8, day: u8) -> RataDie {
    ...
    if year > MIN_NON_LEAP_CORRECTION && NON_LEAP_CORRECTION.contains(&(year - 1)) {
        new_year -= 1;
    }
    RataDie::new(
        new_year - 1
...
pub fn fast_persian_from_fixed(date: RataDie) -> Result<(i32, u8, u8), I32CastError> {
    ...
    if day_of_year == 366 && year >= MIN_NON_LEAP_CORRECTION && NON_LEAP_CORRECTION.contains(&year)
    {
        year += 1;
        day_of_year = 1;
    }
...
pub fn is_leap_year(p_year: i32, _data: ()) -> bool {
    if p_year >= MIN_NON_LEAP_CORRECTION && NON_LEAP_CORRECTION.contains(&p_year) {
        false
    } else if p_year > MIN_NON_LEAP_CORRECTION && NON_LEAP_CORRECTION.contains(&(p_year - 1)) {
        true
    } else {
