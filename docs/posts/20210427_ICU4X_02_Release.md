...
To minimize the size of our WASM binaries we added [`Writeable`], which introduces a lightweight alternative to [`std::fmt::Display`] and [`LiteMap`], which adds a [`std::collections::HashMap`] replacement that uses [`binary_search`] instead of a hash function.
