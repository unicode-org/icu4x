...
Headers can be found under [`ffi/capi/bindings/cpp`], with docs [here](https://unicode-org.github.io/icu4x/docs/ffi/cpp/). The port is header-only; no additional C++ translation units need to be compiled to use ICU4X from C++.
...
- Inside the crate you can use `cargo build` to build the library
 ...
 - Copy the header files from the `icu_capi` crate
    - If you're using the default crate registry, they will be in `~/.cargo/registry/src/*/icu_capi-1.0.0/bindings/cpp`
