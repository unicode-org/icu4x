...
use icu::calendar::{DateTime, Iso, Gregorian};
use icu::datetime::options::length;
use icu::datetime::TypedDateFormatter;
use icu::locid::locale;
use icu_provider_blob::BlobDataProvider;

const CCP_BLOB_PATH: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/data/ccp_gregory.blob");
...
fn main() {
    ...
    let date_formatter = if locale == locale!("ccp") {
        ...
        TypedDateFormatter::<Gregorian>::try_new_with_length_with_buffer_provider(
            &provider,
            &(&locale).into(),
            length::Date::Medium,
        )
    ...
    println!(
        "Date: {}",
        date_formatter
            .format(&iso_datetime.to_calendar(Gregorian))
    );
}
