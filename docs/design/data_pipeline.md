...
The actual resource keys and the structs to which they correspond should be defined in a central location in the repository: [components/data_provider/src](https://github.com/unicode-org/icu4x/tree/master/components/data_provider/src).  Follow conventions of existing data provider struct definitions when adding a new one.
