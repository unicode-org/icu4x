...
fn test_full_mappings() {
    ...
    assert_eq!(
        case_mapping.to_uppercase_string(lowercase_greek, &root),
        uppercase_greek
    );
    assert_eq!(
        case_mapping.to_lowercase_string(uppercase_greek, &root),
        lowercase_greek
    );
    assert_eq!(
    ...
    assert_eq!(
        case_mapping.to_lowercase_string(uppercase_turkish_1, &root),
        "i\u{307}stanbul, not constantinople"
    );
    assert_eq!(
        case_mapping.to_lowercase_string(uppercase_turkish_1, &tr),
        lowercase_turkish_1
    );
    ...
    assert_eq!(
        case_mapping.to_uppercase_string(lowercase_turkish_2, &root),
        "TOPKAPI PALACE, ISTANBUL"
    );
    assert_eq!(
        case_mapping.to_uppercase_string(lowercase_turkish_2, &tr),
        uppercase_turkish_2
    );
    ...
    assert_eq!(
        case_mapping.to_uppercase_string(initial_german, &root),
        uppercase_german
    );
    ...
    assert_eq!(case_mapping.to_lowercase_string(before, &root), after);
    assert_eq!(case_mapping.to_lowercase_string(before, &tr), after_turkish);
    ...
    assert_eq!(case_mapping.to_uppercase_string(before, &root), after);
    assert_eq!(case_mapping.to_uppercase_string(before, &tr), after_turkish);
    ...
    assert_eq!(
        case_mapping.to_uppercase_string(initial_deseret, &root),
        upper_deseret
    );
    assert_eq!(
        case_mapping.to_lowercase_string(initial_deseret, &root),
        lower_deseret
    );
    ...
    assert_eq!(
        case_mapping.to_uppercase_string(initial_ligature, &root),
        upper_ligature
    );
    assert_eq!(
        case_mapping.to_lowercase_string(initial_ligature, &root),
        lower_ligature
    );
    ...
    assert_eq!(
        case_mapping.to_uppercase_string(initial_sigmas, &root),
        upper_sigmas
    );
    assert_eq!(
        case_mapping.to_lowercase_string(initial_sigmas, &root),
        lower_sigmas
    );
    ...
    assert_eq!(case_mapping.to_lowercase_string(initial_dots, &root), after);
    assert_eq!(
        case_mapping.to_lowercase_string(initial_dots, &tr),
        after_turkish
    );
    ...
    assert_eq!(case_mapping.to_uppercase_string(initial_dots, &root), after);
    assert_eq!(
        case_mapping.to_uppercase_string(initial_dots, &lt),
        after_lithuanian
    );
    ...
    assert_eq!(case_mapping.to_lowercase_string(initial_dots, &root), after);
    assert_eq!(
        case_mapping.to_lowercase_string(initial_dots, &lt),
        after_lithuanian
    );
...
fn test_armenian() {
    ...
    assert_eq!(cm.to_uppercase_string(s, &root), "ԵՒ ԵՐԵՒԱՆԻ");
    assert_eq!(cm.to_uppercase_string(s, &east), "ԵՎ ԵՐԵՎԱՆԻ");
    assert_eq!(cm.to_uppercase_string(s, &west), "ԵՒ ԵՐԵՒԱՆԻ");

    // Titlecase doesn't work yet
    // assert_eq!(cm.to_titlecase_string(s, &root), "Եւ Երևանի");
    // assert_eq!(cm.to_uppercase_string(s, &east), "Եվ Երևանի");
    // assert_eq!(cm.to_uppercase_string(s, &west), "Եւ Երևանի");
}
