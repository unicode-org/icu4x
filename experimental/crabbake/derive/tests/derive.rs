...
fn test_int_example() {
    assert_eq!(
        IntExample { x: 17 }.bake().to_string(),
        quote! { IntExample { x: 17u8, }}.to_string()
    );
}
...
fn test_generics_example() {
    assert_eq!(
        GenericsExample { x: 17, y: 100isize }.bake().to_string(),
        quote! { GenericsExample { x: 17u32, y: 100isize, }}.to_string()
    );
}
