    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_relativetime::provider::LongDayRelativeTimeFormatDataV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_relativetime::provider::LongDayRelativeTimeFormatDataV1Marker>, icu_provider::DataError> {
                ...
                static VALUES: [&<icu_relativetime::provider::LongDayRelativeTimeFormatDataV1Marker as icu_provider::DataMarker>::Yokeable; 93usize] = [&AF, &AM, &AR, &AS, &AZ, &BE, &BG, &BN, &BS, &CA, &CS, &CY, &DA, &DE, &EL, &EN, &ES, &ET, &EU, &FA, &FI, &FIL, &FR, &GA, &GD, &GL, &GU, &HA, &HE, &HI, &HI_LATN, &BS, &HU, &HY, &ID, &IG, &IS, &IT, &JA, &JV, &KA, &KK, &KM, &KN, &KO, &KOK, &KY, &LO, &LT, &LV, &MK, &ML, &MN, &MR, &MS, &MY, &NE, &NL, &NN, &NO, &OR, &PA, &PCM, &PL, &PS, &PT, &RO, &RU, &SD, &SI, &SK, &SL, &SO, &SQ, &SR, &SR_LATN, &SV, &SW, &TA, &TE, &TH, &TK, &TR, &UK, &UR, &UZ, &VI, &YO, &YUE, &YUE_HANS, &ZH, &ZH_HANT, &ZU];
                ...
                match KEYS.binary_search_by(|k| locale.strict_cmp(k.as_bytes()).reverse()) {
                    ...
                    Err(_) => Err(icu_provider::DataErrorKind::MissingLocale.with_req(<icu_relativetime::provider::LongDayRelativeTimeFormatDataV1Marker as icu_provider::KeyedDataMarker>::KEY, req)),
                }
            }
        }
    };
