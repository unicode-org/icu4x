...
pub(crate) struct ParserOptions {
    ...
    /// like "{0} 'days'".
    pub(crate) allow_raw_letters: bool,
}
...
impl<'p, P> Parser<'p, P> {
    ...
    /// );
    /// ```
    pub(crate) fn new(input: &'p str, options: ParserOptions) -> Self {
        Self {
            input,
            len: input.len(),
...
mod tests {
    ...
    fn pattern_parse_placeholders() {
        ...
        for (input, expected) in samples {
            ...
            let mut result = vec![];
            while let Some(item) = parser.try_next().expect("Failed to parse a pattern") {
                result.push(item);
            }
        ...
        for (input, maybe_error) in broken {
            let mut parser = Parser::<usize>::new(
                input,
                ParserOptions {
                    allow_raw_letters: false,
            ...
            loop {
                match parser.try_next() {
                    Ok(Some(_)) => (),
                    Ok(None) => {
                        // parsed entire pattern (bad)
                        assert!(false, "Should have failed to parse pattern: {input}");
                    }
                    Err(actual_error) => {
                        // found error in pattern
