...
fn raise_annotation<F: Copy, const L: usize, const L1: usize>(top_level: F, mut lower_levels: Vec<Annotation<F, L1>>) -> Vec<Annotation<F,L>> {
    assert_eq!(L - 1, L1);
    
    // Transforms an Annotation<F, L-1> into Annotation<F, L> by prepending the given level
    fn add_level<F: Copy, const L: usize, const L1: usize>(lower_levels: &mut Annotation<F, L1>, top_level: (LocationInPart, F)) -> Annotation<F, L> {
        assert_eq!(L - 1, L1);
    ...
    match lower_levels.len() {
        0 => vec![],
        1 => vec![add_level(&mut lower_levels[0], (LocationInPart::Single, top_level))],
        n => {
            let mut vec: Vec<Annotation<F, L>> = Vec::with_capacity(n);
            vec.push(add_level(&mut lower_levels[0], (LocationInPart::Beginning, top_level)));
            for j in 1..n-1 {
                vec.push(add_level(&mut lower_levels[j], (LocationInPart::Inside, top_level)));
            }
            vec.push(add_level(&mut lower_levels[n-1], (LocationInPart::End, top_level)));
            vec
        }
    }
}
...
mod test {
    ...
    pub enum Field { Word, Space, Greeting }
    ...
    fn test_enum_packing() {
        enum FieldWithIndex { Empty, Index(NonZeroU8) }
