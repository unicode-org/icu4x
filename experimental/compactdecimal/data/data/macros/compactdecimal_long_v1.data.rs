    ...
    ($ provider : path) => {
        ...
        impl icu_provider::DataProvider<icu_compactdecimal::provider::LongCompactDecimalFormatDataV1Marker> for $provider {
            fn load(&self, req: icu_provider::DataRequest) -> Result<icu_provider::DataResponse<icu_compactdecimal::provider::LongCompactDecimalFormatDataV1Marker>, icu_provider::DataError> {
                ...
                static VALUES: [&<icu_compactdecimal::provider::LongCompactDecimalFormatDataV1Marker as icu_provider::DataMarker>::Yokeable; 120usize] = [&AF, &AM, &AR, &AR, &AS, &AS, &AZ, &BE, &BG, &BN, &BN, &BS, &CA, &CS, &CY, &DA, &DE, &EL, &EN, &ES, &ET, &EU, &FA, &FA, &FI, &FIL, &FR, &GA, &GD, &GL, &GU, &GU, &HA, &HE, &HI, &EN, &HI, &HR, &HU, &HY, &ID, &IG, &IS, &IT, &JA, &JV, &JV, &KA, &KK, &KM, &KM, &KN, &KN, &KO, &KOK, &KOK, &KY, &LO, &LO_U_NU_LAOO, &LT, &LV, &MK, &ML, &ML, &MN, &MR, &MR, &MS, &MY, &MY, &NE, &NE, &NL, &NN, &NO, &OR, &OR, &PA, &PA, &PCM, &PL, &IG, &IG, &PT, &RO, &RU, &SD, &SD, &SI, &SK, &SL, &SO, &SQ, &SR, &BS, &SV, &SW, &TA, &TA, &TE, &TE, &TH, &TH, &TK, &TR, &UK, &UR, &UR, &UZ, &VI, &YO, &YUE, &YUE_HANS, &YUE_HANS, &YUE, &ZH, &ZH_HANT, &ZH_HANT, &ZH, &ZU];
                ...
                match KEYS.binary_search_by(|k| locale.strict_cmp(k.as_bytes()).reverse()) {
                    ...
                    Err(_) => Err(icu_provider::DataErrorKind::MissingLocale.with_req(<icu_compactdecimal::provider::LongCompactDecimalFormatDataV1Marker as icu_provider::KeyedDataMarker>::KEY, req)),
                }
            }
        }
    };
