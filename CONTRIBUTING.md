...
Our wider testsuite is organized as `ci-job-foo` make tasks corresponding to each GitHub Actions CI job, and you can run any testsuites you consider relevant:
 ...
 - `cargo make ci-job-ffi`: Runs all of the FFI tests; mostly important if you're changing the FFI interface. This has several additional dependencies:
     + Rust toolchain `nightly-2021-12-09`: `rustup install nightly-2021-12-09`
         * `rust-src` for that toolchain: `rustup component add --toolchain nightly-2021-12-09 rust-src`
         * Various targets for that toolchain: `rustup target add thumbv7m-none-eabi --toolchain nightly-2021-12-09`, `rustup target add thumbv8m.main-none-eabihf --toolchain nightly-2021-12-09`, `rustup target add x86_64-unknown-linux-gnu --toolchain nightly-2021-12-09`
 ...
 - `cargo make ci-job-wasm`: Runs WASM tests; mostly important if you're changing the FFI interface. This also has a couple additional dependencies:
     ...
     + Rust toolchain `nightly-2021-12-09`: `rustup install nightly-2021-12-09`
         * `rust-src` for that toolchain: `rustup component add --toolchain nightly-2021-12-09 rust-src`
         * Various WASM targets for that toolchain: `rustup target add wasm32-unknown-unknown --toolchain nightly-2021-12-09`, `rustup target add wasm32-unknown-emscripten --toolchain nightly-2021-12-09`
